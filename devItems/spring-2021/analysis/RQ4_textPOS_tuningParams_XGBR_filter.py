...
fopOutputAllSystems=fopOutput+'/RQ4_tuning_xgbr_filter/'
...
for mIndex in range(0, len(param_grid['max_depth'])):
    for mcwIndex in range(0, len(param_grid['min_child_weight'])):
        for etaIndex in range(0, len(param_grid['eta'])):
            for alpIndex in range(0, len(param_grid['alpha'])):
                for lrIndex in range(0, len(param_grid['learning_rate'])):
                    lstMAE = []
                    lstValMAE = []
                    valMItem=param_grid['max_depth'][mIndex]
                    valMcwItem=param_grid['min_child_weight'][mcwIndex]
                    valEtaItem = param_grid['eta'][etaIndex]
                    ...
                    for i in range(0,len(list_files)):
                       ...
                       # # print('{}\t{}'.format(type(X_train),type(y_train)))
                        ...
                        regressor=XGBRegressor(max_depth=valMItem,min_child_weight=valMcwItem,eta=valEtaItem)
                    ...
                    fnTuneName='-'.join([str(valMItem),str(valMcwItem),str(valEtaItem)])
