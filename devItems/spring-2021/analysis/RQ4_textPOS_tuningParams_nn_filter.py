...
fopOutputAllSystems=fopOutput+'/RQ4_tuning_nn_filter/'
...
for hlsIndex in range(0, len(param_grid['hidden_layer_sizes'])):
    for aIndex in range(0, len(param_grid['activation'])):
        for sIndex in range(0, len(param_grid['solver'])):
            for alpIndex in range(0, len(param_grid['alpha'])):
                for lrIndex in range(0, len(param_grid['learning_rate'])):
                    lstMAE = []
                    lstValMAE = []
                    valHlsItem=param_grid['hidden_layer_sizes'][hlsIndex]
                    valAItem=param_grid['activation'][aIndex]
                    valSItem = param_grid['solver'][sIndex]
                    valAlpItem = param_grid['alpha'][aIndex]
                    valLrItem = param_grid['learning_rate'][sIndex]
                    ...
                    fnTuneName='-'.join([str(valHlsItem),str(valAItem),str(valSItem),str(valAlpItem),str(valLrItem)])
