...
fopOverallResultReg= 'result_RealSVR_D2v/'
fopDataset='../../dataset/'
...
for file in arrFiles:
   ...
   # fnAll='_10cv.csv'
    ...
    for c in Cs:
        for gamma in gammas:
            for k in kernels:
                clf=SVR(C=c,gamma=gamma,kernel=k)
                ...
                if(maeAccuracy<bestMAE):
                    ...
                    bestGamma=gamma
                    bestKernel=k
    clf=SVR(C=bestC,gamma=bestGamma,kernel=bestKernel)
    clf.fit(X_train,y_train)
    predicted = clf.predict(X_test)
    predicted = convertTopLabelToNormalLabel(predicted)
    maeAccuracy = mean_absolute_error(y_test, predicted)
    filePredict = ''.join([fopOutputItemDetail, file, '_RealSVR.txt'])
    np.savetxt(filePredict, predicted, fmt='%s', delimiter=',')

    o3 = open(fpMAEMin, 'a')
    o3.write('{}\t{}\t{}\n'.format(file, 'RealSVR', maeAccuracy))
    o3.close()
