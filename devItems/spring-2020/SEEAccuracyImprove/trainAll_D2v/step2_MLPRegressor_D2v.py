...
fopOverallResultReg= 'result_MLPR_D2v/'
fopDataset='../../dataset/'
...
for file in arrFiles:
   ...
   # fnAll='_10cv.csv'
    ...
    hidden_layer_sizes=[(5),(7),(10),(5,),(10,)]
    activation=[ "logistic", "tanh", "relu"]
    solver=["sgd","adam"]
    alpha= [0,0.0000000001, 0.000000001,0.00000001,0.0000001,0.0000005,0.000001,0.000005,0.00005,0.0005,0.005,0.05]
    ...
    filePredict = ''.join([fopOutputItemDetail, file, '_MLPR.txt'])
    for hd in hidden_layer_sizes:
        for act in activation:
            for so in solver:
                for al in alpha:
                    clf=MLPRegressor(max_iter=100000,hidden_layer_sizes=hd,activation=act,alpha=al,solver=so)
                    clf.fit(X_train,y_train)
                    predicted = clf.predict(X_test)

                    predicted = convertTopLabelToNormalLabel(predicted)
                    maeAccuracy = mean_absolute_error(y_test, predicted)
                    print('{}\t{}\t{}\t{}\t{}\t{}'.format(hd, act, so, al,maeAccuracy,bestMAE))
                    if(maeAccuracy<bestMAE):
                        ...
                        o2 = open(fpResultAll, 'w')
                        o2.write('Result for ' + str(clf) + '\n')
                        o2.write('MAE {}\n'.format(maeAccuracy))
                        o2.close()

    o3 = open(fpMAEMin, 'a')
    o3.write('{}\t{}\t{}\n'.format(file, 'MLPR', bestMAE))
    o3.close()

    # clf=MLPRegressor(max_iter=100000,hidden_layer_sizes=bestHd,activation=bestAct,alpha=bestAl,solver=bestSo)
    # clf.fit(X_train,y_train)
    # predicted = clf.predict(X_test)
    # predicted = convertTopLabelToNormalLabel(predicted)
    # maeAccuracy = mean_absolute_error(y_test, predicted)
