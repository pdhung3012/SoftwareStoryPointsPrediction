...
fopOverallResultReg= 'result_XGBR_D2v/'
fopDataset='../../dataset/'
...
for file in arrFiles:
   ...
   # fnAll='_10cv.csv'
    ...
    random_seed=2
    learning_rate_arr =[0.05, 0.10, 0.15, 0.20, 0.25, 0.30]
    max_depth_arr= [3, 4, 5, 6, 8, 10, 12, 15]
    min_child_weight_arr= [1, 3, 5, 7]
    gamma_arr= [0.0, 0.1, 0.2, 0.3, 0.4]
    colsample_bytree_arr= [0.3, 0.4, 0.5, 0.7]
    ...
    for lr in learning_rate_arr:
        for md in max_depth_arr:
            for mcw in min_child_weight_arr:
                for g in gamma_arr:
                    for cb in colsample_bytree_arr:
                        clf=xgb.XGBRegressor(objective ='reg:squarederror', colsample_bytree = cb, learning_rate = lr,
                max_depth = md,min_child_weight=mcw,gamma=g, alpha = 10, n_estimators = 10)
                        ...
                        if(maeAccuracy<bestMAE):
                            ...
                            bestGamma = g
                            bestCB = cb
    clf=xgb.XGBRegressor(objective ='reg:squarederror', colsample_bytree = bestCB, learning_rate = bestLR,
                max_depth = bestMD,min_child_weight=bestMinCW,gamma=bestGamma, alpha = 10, n_estimators = 10)
    clf.fit(X_train,y_train)
    predicted = clf.predict(X_test)
    predicted = convertTopLabelToNormalLabel(predicted)
    maeAccuracy = mean_absolute_error(y_test, predicted)
    filePredict = ''.join([fopOutputItemDetail, file, '_XGBR.txt'])
